...
class Modules {
        ...
        context.dir(manifestDirectory)
        {
            ...
            if (projects.size() > 0) {
                for (int i = 0; i < projects.size(); i++)
                {
                    ...
                    context.bat "\"${context.tool 'DefaultMSBuild'}\\msbuild.exe\" \"$project.name\" /nologo /verbosity:m /t:Clean,PackModule /p:Configuration=Release /p:Platform=AnyCPU /p:DebugType=none /p:AllowedReferenceRelatedFileExtensions=.xml \"/p:OutputPath=$tempDir\" \"/p:VCModulesOutputDir=$modulesDir\" \"/p:VCModulesZipDir=$packagesDir\""
                }
            }
        }
    }
    ...
    def static runIntegrationTests(context)
    {
        ...
        // create context
        context.withEnv(["VC_PLATFORM=http://localhost:${platformPort}", "VC_STOREFRONT=http://ci.virtocommerce.com:${storefrontPort}", "VIRTO_CONN_STR_VirtoCommerce=Data Source=http://ci.virtocommerce.com:${sqlPort};Initial Catalog=VirtoCommerce2;Persist Security Info=True;User ID=sa;Password=v!rto_Labs!;MultipleActiveResultSets=True;Connect Timeout=30" ]) {
