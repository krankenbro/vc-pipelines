...
class Packaging {
    ...
    def static startDockerTestEnvironment(context, String dockerTag)
    {
        ...
        context.dir(composeFolder)
        {
            ...
            context.withEnv(["DOCKER_TAG=${dockerTag}", "DOCKER_PLATFORM_PORT=${platformPort}", "DOCKER_STOREFRONT_PORT=${storefrontPort}", "DOCKER_SQL_PORT=${sqlPort}", "COMPOSE_PROJECT_NAME=${context.env.BUILD_TAG}" ]) {
                context.bat "docker-compose down"
                try {
                    def tryNumber = 1
                    context.retry(3){
                        context.echo "Number of containers launch attempts: ${tryNumber}"
                        context.sleep(time: (tryNumber-1)*10, unit: 'SECONDS')
                        tryNumber++
    ...
    def static createReleaseArtifact(context, version, webProject, zipArtifact, websiteDir)
    {
        ...
        {
            context.bat "\"${context.tool 'DefaultMSBuild'}\\msbuild.exe\" \"${webProject}\" /nologo /verbosity:m /p:Configuration=Release /p:Platform=\"Any CPU\" /p:DebugType=none \"/p:OutputPath=$tempFolder\""
        }

        def ant = new AntBuilder()
        ant.zip(destfile: "${packagesDir}\\${zipArtifact}.${version}.zip", basedir: "${websitePath}")
    }
    ...
    def static runBuild(context, solution)
    {
        if(context.projectType == 'NETCORE2')
        {
            ...
            context.bat "\"${tool 'DefaultMSBuild'}\\msbuild.exe\" \"${solution}\" /p:Configuration=Debug /p:Platform=\"Any CPU\" /t:restore /t:rebuild /m"
        }
        else
        {
    ...
    def static startSonarJS(context){
        ...
        context.withSonarQubeEnv('VC Sonar Server') {
            context.bat "\"${sqScannerMsBuildHome}\\sonar-scanner-3.2.0.1227\\bin\\sonar-scanner.bat\" scan -Dsonar.projectKey=theme_default_${context.env.BRANCH_NAME} -Dsonar.sources=./assets -Dsonar.branch=${context.env.BRANCH_NAME} -Dsonar.projectName=\"${fullJobName}\" -Dsonar.host.url=%SONAR_HOST_URL% -Dsonar.login=%SONAR_AUTH_TOKEN% -Dsonar.eslint.reportPaths=\"${context.env.WORKSPACE}@tmp\\report.json\""
            //context.bat "\"${sqScannerMsBuildHome}\\sonar-scanner-3.2.0.1227\\bin\\sonar-scanner.bat\" end -Dsonar.login=%SONAR_AUTH_TOKEN%"
        }
    }
    ...
    def static endAnalyzer(context)
    {
        ...
        def fullJobName = Utilities.getRepoName(context)
        context.withSonarQubeEnv('VC Sonar Server') {
            context.bat "\"${sqScannerMsBuildHome}\\SonarScanner.MSBuild.exe\" end /d:sonar.login=%SONAR_AUTH_TOKEN%" //https://jira.sonarsource.com/browse/SONARMSBRU-378
        }        
    }
    ...
    def static runGulpBuild(context)
    {
        ...
        if(bowerjs.exists()){
            context.bat "bower install --force-latest"
        }
