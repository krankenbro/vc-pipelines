    ...
    */
    ...
    def static startAnalyzer(context)
    {
        ...
        context.withSonarQubeEnv('VC Sonar Server') {
            ...
            context.bat "\"${sqScannerMsBuildHome}\\SonarQube.Scanner.MSBuild.exe\" begin /d:\"sonar.branch=${context.env.BRANCH_NAME}\" /n:\"${fullJobName}\" /k:\"${fullJobName}\" /d:sonar.host.url=%SONAR_HOST_URL% /d:sonar.login=%SONAR_AUTH_TOKEN% /d:sonar.cs.vscoveragexml.reportsPaths=\"${coverageFolder}\\VisualStudio.Unit.coveragexml\"" // -/d:\"sonar.organization=virtocommerce\"
        }        
    }
